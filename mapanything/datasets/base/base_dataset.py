...
class BaseDataset(EasyDataset):
    ...
    def _sample_view_indices(
        ...
        """
        if (
                self.covisibility_thres is None or self.covisibility_thres == 0
        ) and num_views_to_sample < num_views_in_scene:
            # 执行均匀采样 (linspace) 以保证覆盖率
            indices = np.linspace(
                0, num_views_in_scene - 1, num_views_to_sample, dtype=int
            )
            return indices
        if num_views_to_sample == num_views_in_scene:
            ...
            # (此块仅在 covisibility_thres > 0 且 num_views_to_sample < num_views_in_scene 时执行)
