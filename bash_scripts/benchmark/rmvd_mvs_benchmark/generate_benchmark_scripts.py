...
def get_model_settings(model: str, dataset: str):
    if model == "map-anything":
        return {
            "model": "map-anything",
            "model.pretrained": "\\${root_experiments_dir}/map-anything/training/mapa_curri_24v_13d_48ipg_64g/checkpoint-last.pth",
...
def generate_shell_for_single_experiment(
    ...
    benchmark_configs = {
        ...
        "hydra.run.dir": '"\${root_experiments_dir}/map-anything/benchmarking/rmvd_'
...
if __name__ == "__main__":
    ...
    for dataset in ["kitti", "scannet"]:
        # non-conditioned
        for model in ["moge_2", "map-anything"]:
    ...
    for dataset in ["kitti", "scannet"]:
        # non-conditioned
        for model in ["map-anything", "must3r"]:
            generate_shell_for_single_experiment(
                model=model,
                dataset=dataset,
                conditioning="image",
    ...
    for dataset in ["kitti", "scannet"]:
        # non-conditioned
        for model in ["vggt", "map-anything", "must3r"]:
            generate_shell_for_single_experiment(
                model=model,
                dataset=dataset,
                conditioning="image",
