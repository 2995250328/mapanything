...
for combo in "${batch_sizes_and_views[@]}"; do
    ...
    python3 \
        ...
        model=map-anything \
        model/info_sharing=aat_ifr_24_layers_w_view_pe \
        model/task=images_only \
        model.encoder.uses_torch_hub=false \
        model.pretrained='${root_experiments_dir}/map-anything/training_ablations/2b_aat_w_pe/checkpoint-best.pth' \
        hydra.run.dir='${root_experiments_dir}/map-anything/benchmarking_ablations/dense_'"${num_views}"'_view/2b_aat_w_pe'
