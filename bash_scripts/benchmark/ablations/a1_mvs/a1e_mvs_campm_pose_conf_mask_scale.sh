...
for combo in "${batch_sizes_and_views[@]}"; do
    ...
    python3 \
        ...
        model=map-anything \
        model/task=mvs \
        model/pred_head/adaptor_config=campointmap_pose_confidence_mask_scale \
        model.encoder.uses_torch_hub=false \
        model.pretrained='${root_experiments_dir}/map-anything/training_ablations/a1e_campm_pose_conf_mask_scale/checkpoint-best.pth' \
        hydra.run.dir='${root_experiments_dir}/map-anything/benchmarking_ablations/dense_'"${num_views}"'_view/a1e_mvs_campm_pose_conf_mask_scale'
